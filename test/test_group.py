...
def test_insert():
  ...
  setup.app().post('/%s/group', {'name':'group1','type':'type','organization_uid':organization_uid}) 
  setup.app().post('/%s/group', {'name':'group2','type':'type','organization_uid':organization_uid}) 
  setup.app().post('/%s/group', {'name':'group3','type':'type','organization_uid':organization_uid}) 

  res = setup.app().get('/%s/groups' % (key,) ) 
...
def test_update_group():
  ...
  res = setup.app().post('/%s/group' % (key,), {'name':'group14','type':'type2'}) 
  group_uid = str(res.json.get('uid'))
  res = setup.app().get('/%s/group/%s' % (key,group_uid) ) 
  print res.json
  print res.json.get('response')
  assert res.json.get('response').get('type') == 'type2'

  res = setup.app().post('/%s/group/%s' % (key,group_uid), {'name':'group14','type':'mynewtype'})
  print res.json.get('response')
  res = setup.app().get('/%s/group/%s' % (key,group_uid)) 
  print res.json.get('response')
