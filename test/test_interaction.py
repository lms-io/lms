...
def test_update_interaction():
  ...
  res = setup.app().post('/%s/interaction' % (key,), {'name':'interaction14','url':'url2'}) 
  ...
  res = setup.app().post('/%s/interaction/%s' % (key,interaction_uid), {'name':'interaction14','url':'mynewurl'})
  print res.json.get('response')
  res = setup.app().get('/%s/interaction/%s' % (key,interaction_uid)) 
  print res.json.get('response')
