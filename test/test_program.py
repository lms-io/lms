import random, os, uuid, bcrypt, sys, inspect, setup, configparser

from cassandra.cluster import Cluster
from webtest import TestApp

config = configparser.ConfigParser()
config.read('config.ini')

syskey = config.get('application','syskey')

def test_insert():
  res = setup.app().post('/%s/organization' % (syskey,), {'name':'testcorp', 'admin_username':'joe', 'admin_password':'password'})
  organization_uid = str(res.json.get('uid'))

  res = setup.app().post('/auth/login', {'username':'joe','password':'password','organization_uid':organization_uid}) 
  key = res.json.get('session')

  setup.app().post('/%s/program', {'name':'program1','organization_uid':organization_uid}) 
  setup.app().post('/%s/program', {'name':'program2','organization_uid':organization_uid}) 
  setup.app().post('/%s/program', {'name':'program3','organization_uid':organization_uid}) 

  res = setup.app().get('/%s/programs' % (key,) ) 
  print res.json.get('message')
  assert res.json.get('status') == 'OK' 


def test_get_all_programs():
  res = setup.app().post('/%s/organization' % (syskey,), {'name':'testcorp', 'admin_username':'joe', 'admin_password':'password'})
  organization_uid = str(res.json.get('uid'))

  res = setup.app().post('/auth/login', {'username':'joe','password':'password','organization_uid':organization_uid}) 
  key = res.json.get('session')


  res = setup.app().get('/%s/programs' % (key,) ) 
  assert res.json.get('status') == 'OK' 

def test_update_program():
  res = setup.app().post('/%s/organization' % (syskey,), {'name':'testcorp', 'admin_username':'joe2', 'admin_password':'password'})
  organization_uid = str(res.json.get('uid'))

  res = setup.app().post('/auth/login', {'username':'joe2','password':'password'}) 
  key = res.json.get('session')

  res = setup.app().post('/%s/program' % (key,), {'name':'testname'}) 
  program_uid = str(res.json.get('uid'))
  res = setup.app().get('/%s/program/%s' % (key,program_uid) ) 
  print res.json
  print res.json.get('response')
  assert res.json.get('response').get('name') == 'testname'

  res = setup.app().post('/%s/program/%s' % (key,program_uid), {'name':'mynewprogram'})
  print res.json.get('response')
  res = setup.app().get('/%s/program/%s' % (key,program_uid)) 
  print res.json.get('response')
  assert res.json.get('response').get('name') == 'mynewprogram'
