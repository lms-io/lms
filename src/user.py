...
def create(organization_uid, username, password, firstName="", lastName=""):
  ...
  ins = "INSERT INTO user_by_organization (organization_uid, username) VALUES (%s,%s);"
...
def list(organization_uid):
  rows = appcontext.db().execute('SELECT username,organization_uid from user_by_organization where organization_uid=%s', (organization_uid,))
  d = []
  for r in rows:
...
def get(username):
  r = appcontext.db().execute('SELECT username,firstName,lastName,organization_uid from user where username=%s', (username,))
